...
const DetailsBanner = ({ video, crew }) => {
    ...
    // for Shwoing Video on Screen
    const [show, setShow] = useState(false);
    const [videoId, setVideoId] = useState(null);

    const { mediaType, id } = useParams();  //I Get this from Details URL 
    const { data, loading } = useFetch(`/${mediaType}/${id}`);

    const { url } = useSelector(state => state.home);

    // Now Get Genres Id's from data.genres
    const genre = data?.genres?.map(g => g.id);  // Now pass this genre array to <Genre /> to show on screen

    //Now Get Directors & Writers from Crew data
